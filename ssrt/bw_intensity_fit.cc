...
#define PI_MASS 0.139

using namespace std; 
typedef std::complex<double> cd;

#define w(a,b) (b-sqrt(a-b))/(b+sqrt(a-b))

#define A1_MASS 1.26
#define A1_WIDTH 0.021
...
vector<vector<pair<double,cd> > > *arr;
...
cd Afull(cd s, vector<cd> apar, vector<cd> npar, 
  ...
  //Constract D
  cd dsum =0;
  for(int i=0;i<npar.size();i++) dsum += npar[i]*vtable(real(s),(*arr)[i]);
...
double lambda(double a1, double b1, double c1) {
  return a1*a1+b1*b1+c1*c1-2*a1*b1-2*b1*c1-2*c1*a1;
}
...
double BreitWignerI(double x,double m, double G0) {
  ...
  double G = G0*p/p0;
  cd back(2,2);
  cd A = sqrt(m*G)/(m*m-x-unit*m*G) + back;
  return norm(A);
} 
...
int main() {
  ...
  arr=&data;
  cout << "test access " << (*arr)[0][5].second << endl; 
  ...
  //Fit
  double fitRleft=1.2,fitRright=1.8;
  const int NumPar = 2*NPAR_FIT + 2*APAR_FIT - 1;
  TF1 *f = new TF1("fit",intensity,fitRleft,fitRright,NumPar);
  ...
  f2->SetLineColor(46);  f2->Draw("same");
