...
int fit_data(int nAttempt, const char *fout_name) {
  ...
  string var_name[NumPar] = {"a1_mass","a1_g","a1_rc0","a1_ic0",
			     ...
			     "a1p_mass","a1p_g","a1p_rc0","a1p_ic0",
  ...
  vector<int> unused = {3,16};//
  const int NstepsInfit = 3;
  vector<int> fix[NstepsInfit] = { {                        12,13,14,15,            21,22,23,24},
...
int plot_polar(const char *fin_name, int entry) {
  ...
  mt->Add(gDt1_gray,"p");
  mt->Add(gDt1_dark,"p");
  TGraphErrors gtmp1(1);gtmp1.SetPoint(0,530.,0.); //gtmp1.SetPoint(1,530.,450.);
  mt->Add(&gtmp1,"pl");
  mt->Draw("a");
  const int Nlines = 10;
  for(int i=0;i<Nlines;i++) {
    ...
    l->SetLineColor(kGreen); l->SetAngle(60); 
    l->Draw();
  }
  mt->GetHistogram()->SetTitle("1^{++}0^{+} f_{0}#pi P: breit-wigner function + background;real(A);imag(A)");
  mt->GetHistogram()->GetYaxis()->SetRangeUser(-320.,480.);
...
//  double shift = pars[16];
  ...
  double m1 = pars[8];
  double g1 = pars[9];
  cd c1(pars[10],pars[11]);
  cd bw1 = c1*a1_bw(s,m1,g1,glookup_rho_RHO,1,5.0);
  if(flag==1) return bw1;
...
void scalePars(double *start_pars) { 
  ...
  start_pars[10] *= sqrt(gNorm1); start_pars[11] *= sqrt(gNorm1); start_pars[14] *= sqrt(gNorm1); start_pars[15] *= sqrt(gNorm1);
